	...
	bool espace = true;
     ...
     while(keyPressed = getch()){
		...
		if(keyPressed > 48 && keyPressed < 58){
			int numeroIntroducido = (int) keyPressed - 48;
			numeros += to_string(numeroIntroducido);

			espace = false;

			if(numeroIntroducido == 1 && negative){
				numeros += '\0';
			...
			negative = false;
		}else if (keyPressed == ' ' && !espace && !negative){
...
void printCadenaActual (string cadenaCompletada, string cadena_add){
     ...
     cout << azul << cadena_add << restaurar << endl;
}
